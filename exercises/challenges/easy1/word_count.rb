...
class Phrase
  ...
  def normalize(word)
    normalized = word.scan(/\b[\w']+\b/).join.downcase
...
class PhraseTest < Minitest::Test
  ...
  def test_with_word_ending_with_apostrophe
    phrase = Phrase.new("Louis' burgers made by 'Louis'.")
    counts = {
      "louis'" => 1, "burgers" => 1, 'made' => 1,
      'by' => 1, 'louis' => 1
    }
    assert_equal counts, phrase.word_count
  end
