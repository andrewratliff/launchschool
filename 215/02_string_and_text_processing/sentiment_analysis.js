...
let isPositiveWord = word => positiveRegex.test(word);
let isNegativeWord = word => negativeRegex.test(word);

let sentiment = text => {
  let wordList = text.toLowerCase().match(/[a-z']+/g);

  let positiveWords = wordList.filter(isPositiveWord);
  let negativeWords = wordList.filter(isNegativeWord);
  ...
  return `
  There are ${positiveWords.length} positive words in the text.
  Postive sentiments: ${positiveWords.join(', ')}
